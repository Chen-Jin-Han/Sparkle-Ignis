...
Sparkle is a technical research and report generation multi-agent workflow system adapted from the open-source [GPT Researcher](https://github.com/assafelovic/gpt-researcher) project.

This fork adds a local, interview-ready Sparkle workflow with fire-themed AI agents:
Ignis, Ember, Flare, Blaze, Cinder, Forge, Kindle, and Pyre.

Run the local workflow without API keys:

```bash
python -m sparkle.cli
```

Run the warm local web demo with Docker:
...
To enable DeepSeek polishing, copy `.env.sparkle.example` to `.env.sparkle`, set `SPARKLE_USE_DEEPSEEK=true`, and put your key in `DEEPSEEK_API_KEY`.

For the production LangGraph/GPT Researcher workflow, configure the normal LLM and retriever environment variables, then run:

```bash
python multi_agents/main.py
```

Sparkle's default task is configured for a Chinese technical report covering research planning, source retrieval, parallel section drafting, review, revision, writing, and publishing.
